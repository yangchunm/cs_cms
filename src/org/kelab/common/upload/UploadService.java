...
import org.kelab.util.DateUtils;
...
public class UploadService {
	 ...
	 * ueditor 上传业务方法
	 */
	public Ret ueditorUpload(KeUser account, String uploadType, UploadFile uf) {
		...
		buildPathAndFileName(account.getId(), extName, relativePathFileName, absolutePathFileName, fileName);
		System.out.println(absolutePathFileName[0]);
	...
	private void buildPathAndFileName(
		...
		String relativePath = PropKit.get("cmsImage")+"" + DateUtils.todayToString() + "/";    // 生成相对对路径

		fileName[0] = generateFileName(accountId, extName);
		relativePathFileName[0] =  relativePath + fileName[0];

		String absolutePath = PathKit.getWebRootPath()+"/"+PropKit.get("baseUploadPath")+"" + relativePath;   // webRootPath 将来要根据 baseUploadPath 调整，改代码，暂时选先这样用着，着急上线
