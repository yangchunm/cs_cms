...
public class NaviAdminService {
	public static final NaviAdminService me = new NaviAdminService();
	 ...
	 * @return
	 */
	public CmsNavi findTopCmsNavi(int naviId){
		CmsNavi navi = dao.findById(naviId);
		if(navi.getNaviParentId() == 0)
			return navi;
		else
			return findTopCmsNavi(navi.getNaviParentId());
	}
	...
	public Ret save(CmsNavi cmsNavi,int userId){
		...
		NaviAdminService.me.clearCache();    // 清缓存
		return Ret.ok();
	}
	...
	public CmsNavi findFirstChild(int naviId){
		String sql = "select * from cms_navi where navi_parent_id =? order by navi_order asc limit 1";
		List<CmsNavi> naviL = dao.find(sql,naviId);
		if(naviL.size()>0)
			return naviL.get(0);
		else
			return null;
	 ...
	 * @return
	 */
	public List<CmsNavi> findParentByNavi(CmsNavi navi){
		List<CmsNavi> naviParentL = new ArrayList<CmsNavi>();
		naviParentL.add(navi);
		if(navi.getNaviParentId() != 0){
			while(navi.getNaviParentId() != 0){
				
				navi = CmsNavi.dao.findById(navi.getNaviParentId());
				naviParentL.add(navi);
			}
			Collections.reverse(naviParentL);
		}	
		return naviParentL;
	}
